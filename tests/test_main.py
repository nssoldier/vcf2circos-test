...
def test_get_copynumber_type():
    ...
    for variants in pc.data["Record"]:
        assert pc.get_copynumber_type(variants)[0] in options["Color"].keys(), "unknown SVtype"
...
def test_get_genes_var():
    ...
    for variants in pc.data["Record"]:
        if variants.CHROM == "chr14" and variants.POS == 50000000:
            ...
            assert all(genes for genes in genes_list if genes in genes_check) and len(
                genes_list
            ) == len(genes_check), (
                "ERROR Genes finding have a problem \nGenes needed: "
                + str(len(genes_check))
                + " genes \nGenes: "
                + " ,".join(genes_check)
                + "\nGenes provided: "
...
def test_datafactory_type():
    ...
    assert plots == plots_expected, (
        "ERROR wrong category chart in list \nNeeded: "
        + " ,".join(plots_expected)
        + "\nProvided: "
        + " ,".join(plots)
    )
...
def test_empty_vcf():
    ...
    assert (
        len(pc.data["Variants"]) == 0
    ), "No variant should be present, variants at least one " + str(pc.data["Variants"][0])
