...
app.layout = html.Div(
    [
        ...
        dcc.Upload(
            id="upload-data", children=html.Div(["Drag and Drop or ", html.A("Select Files")])
        ),
...
@app.callback(Output("output-upload", "children"), [Input("upload-data", "contents")])
...
@app.callback(Output("single_output_0", "style"), [Input("number", "value")])
def toggle_0(number):
    if number >= 1:
...
@app.callback(Output("single_output_1", "style"), [Input("number", "value")])
def toggle_1(number):
    if number >= 2:
        return {"display": "block"}
    else:
...
@app.callback(Output("single_output_2", "style"), [Input("number", "value")])
def toggle_2(number):
    if number >= 3:
        return {"display": "block"}
    else:
...
@app.callback(Output("single_output_3", "style"), [Input("number", "value")])
def toggle_3(number):
    if number >= 4:
        return {"display": "block"}
    else:
...
@app.callback(Output("single_output_4", "style"), [Input("number", "value")])
def toggle_4(number):
    if number >= 5:
        return {"display": "block"}
    else:
...
def display_result(number, *arg):
    ...
    for i in range(number):
        di = {"input": arg[2 * i], "radio": arg[2 * i + 1]}
        res.append(di)
...
{
    "file": {"path": "<_io.StringIO object at 0x11cdbe4c8>", "sep": "\t", "header": "infer"},
    "hovertextformat": ' "Chromosome: {}<br>Position: {}<br>Value: {:.2f}".format(a[i,0], a[i,1], float(a[i,2])) ',
    "radius": {"R0": 0.4, "R1": 0.5},
    "marker": {"size": 2},
    "line": {"width": 2, "opacity": 0.9, "smoothing": 0},
    ...
    "trace": {
        ...
        "line": {"color": "black", "width": 2, "shape": "linear", "smoothing": 0},
    },
}
